...
import { plainTitle } from '@/components/kunskap/EmphasisTitle'
import { CATEGORY_LABEL, formatDate, getArticles, isRevised } from '@/lib/kunskap'
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      type: 'website',
      url: 'https://elivro.se/kunskap',
      title: 'Kunskap — Elivro',
      description:
...
export default async function KunskapIndex() {
  const articles = await getArticles('kunskap')
      ...
      <div data-surface="light">
        ...
        <header className="mx-auto max-w-7xl px-6 lg:px-12 pt-20 pb-14 md:pt-28 md:pb-20">
          <div className="max-w-3xl">
            <p className="kunskap-kicker mb-6">Kunskap</p>
            <h1 className="mb-7">
              Kunskap för dig som inte har tid att <em>gissa</em>.
            </h1>
          ...
          {articles.length === 0 ? (
            <div className="border-t border-[var(--line-strong)] pt-10">
              <p className="text-[17px] leading-[1.55] text-n-600 max-w-[58ch]">
                De första artiklarna publiceras inom kort. Vill du veta när de är uppe,{' '}
                <a
                ...
                return (
                  <li key={article.slug} className="border-b border-[var(--line)]">
                    <Link
                      ...
                      className="kunskap-row group block py-8 md:py-10"
                    >
                      <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2 mb-3">
                        <span className="kunskap-kicker">
                          {CATEGORY_LABEL[article.category] ?? article.category}
                        </span>
                        <span className="kunskap-meta">
                          {revised ? 'Uppdaterad ' : ''}
                      ...
                      {/* Semantically an h2 under the page h1; scaled down in
                          kunskap-prose.css because the bare h2 rule is a
                          64px section-opener. */}
                      <h2 className="kunskap-row-title mb-3">{plainTitle(article.title)}</h2>
